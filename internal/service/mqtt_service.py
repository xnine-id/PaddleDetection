...
class MQTTService:
    ...
    def __init__(self, config):
        ...
        self.event_topic = self.config.get('event_topic_prefix', 'fightdetection/events')
        self.cmd_prefix = self.config.get('command_topic_prefix', 'fightdetection/cmd')
        self.state_prefix = self.config.get('state_topic_prefix', 'fightdetection/state')
    ...
    def publish_state(self, cam_name, is_running):
        ...
        logger.debug(f"[MQTT] State: {cam_name} is {'RUNNING' if is_running else 'STOPPED'}")

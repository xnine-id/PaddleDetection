...
from internal.services.camera_service import CameraService
from internal.api.schemas import AddCameraRequest, UpdateCameraRequest, CameraResponse, GenericResponse
...
def get_camera_router(camera_manager: CameraManager):
    ...
    async def sync_cameras(request: List[AddCameraRequest], service: CameraService = Depends(get_camera_service)):
        cameras = await service.sync_cameras(request)
        return cameras
